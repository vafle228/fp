...
public class BoringWordsFilter : IWordsFilter
{
    ...
    public Result<List<string>> ApplyFilter(List<string> words)
        => File.Exists(ENGLISH_DIC) && File.Exists(ENGLISH_AFF) 
            ? WordList.CreateFromFiles(ENGLISH_DIC, ENGLISH_AFF).AsResult()
    ...
    private static bool IsBoring(string word, WordList wordList)
    {
        ...
        return IsInWordList(details) && IsBoringMorph(details);
    }

    private static bool IsInWordList(WordEntryDetail[] details)
        => details.Length != 0 && details[0].Morphs.Count != 0;

    private static bool IsBoringMorph(WordEntryDetail[] details)
        => details[0].Morphs[0] is "po:pronoun" or "po:preposition" or "po:determiner" or "po:conjunction";
}